...
function AddTask(data) {
    ...
    <div class="vehicle-info">
        <p style="color:#bdbec9;margin-top: 1.4vh;">Reward: <span style="color: green">${data.price} BNE</span></p>
    </div>
   ...
   <div style="display: grid;grid-row-gap: 8px;position: absolute;top: 120%;left: 45%;transform: translate(-50% );width: 80%;">
        <button class="btn-boosting" data-task="start" data-pshit=${data.id} ${disabled} data-id=${data.id} style="color: white;background-color:#2f303b;width: 100%;">${text}</button>
...
$(document).ready(function() {
    ...
    $(document).on('click',".btn-dialog", function(){
        ...
        if($(this).data('idk') == 'normal') {
            close()
            $.post('https://bropixel-boosting/dick', JSON.stringify({
                id: id,
            }));
            ...
            close()
            $.post('https://bropixel-boosting/vin', JSON.stringify({
                id: id,
            }));
    ...
    $("#background-apply").click(function() {
        $.post('https://bropixel-boosting/updateurl', JSON.stringify({url: $("#bacgkroundurl").val()}));

        $(".tablet-container").css('background-image' ,"url(" +  $("#bacgkroundurl").val() + ")")
    })

    $("#background-reset").click(function() {
        $.post('https://bropixel-boosting/updateurl', JSON.stringify({url: dddurl}));
	...
	if(task === null) {
        ...
        if(task === 'start') {
            ...
            $.post('https://bropixel-boosting/decline', JSON.stringify({
                id: id,
            }));
        }
    ...
    $(document).keyup(function(e) {
        ...
        if (e.key === "Escape") {
            close()
            $.post('https://bropixel-boosting/close', JSON.stringify({}));
       }
   });
