    ...
    CoreName.Functions.CreateCallback('bropixel-boosting:removeBNE', function(source, cb , amount)
        ...
        if pData ~= nil then
            ...
            if sql[1].BNE ~= nil then
                ...
                local sql = SQL('UPDATE bropixel_boosting SET BNE=@bne WHERE citizenid=@citizenid', {['@citizenid'] = cid , ['@bne'] = pBNE - amount})
            else
                cb(0)
            end
        end
...
AddEventHandler('bropixel-boosting:AddVehicle', function(model, plate)
    if Config['General']["Core"] == "QBCORE" then
        local src = source
        local pData = CoreName.Functions.GetPlayer(src)
        local cid = pData.PlayerData.citizenid
        ...
        SQL('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state) VALUES (?, ?, ?, ?, ?, ?, ?)',{license, cid, vehicle, hash, vehicleMods, vehicleplate, vehiclestate})
    ...
    elseif Config['General']["Core"] == "ESX" then
        ...
        local vehicleplate = plate
        SQL('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (?, ?, ?)',{steam, vehicleplate, vehicle})
        
    elseif Config['General']["Core"] == "NPBASE" then
        local src = source
        local user = exports[Config['CoreSettings']["NPBASE"]["Name"]]:getModule("Player"):GetUser(src)
        local cid = user:getCurrentCharacter().id
        local steam = user:getVar("hexid")
        local vehicle = model
        local vehicleplate = plate
        local vehiclestate = 1
        local v = {
...
RegisterServerEvent("bropixel-boosting:CallCopsNotify" , function(plate , model , color , place)
    if Config['General']["Core"] == "QBCORE" then
        for k, v in pairs(CoreName.Functions.GetPlayers()) do
            local Player = CoreName.Functions.GetPlayer(v)
            if Player ~= nil then
                if Player.PlayerData.job.name == Config['General']["PoliceJobName"] then
                    TriggerClientEvent("bropixel-boosting:SendNotify" ,Player.PlayerData.source , {plate = plate , model = model , color = color , place = place})
                end
            end
        end
    elseif Config['General']["Core"] == "ESX" then
