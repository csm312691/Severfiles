...
AddEventHandler('bropixel-boosting:CreateContract' , function(shit)
  ...
  local VehiclePrice = Config['Utils']["ClassPrices"][response.c]
  if(shit == nil) then
    shit = false
  else
    shit = true
  ...
  if OnTheDropoffWay then
    ...
    elseif(vinstarted == false) then
      if(DisablerTimes < 4) then
        DisablerUsed = true
        local minigame = exports['boostingminigame']:Open()   
...
AddEventHandler("bropixel-boosting:ContractDone" , function()
  if CompletedTask then
    ...
    BNEBoosting['functions']. AddBne(Config['Utils']["VIN"]["AmountBneAfterDropOff"])
...
AddEventHandler('bropixel-boosting:SendNotify' , function(data)
  if(Config['Utils']["Laptop"]["ShowNotifyOnlyIfHasItem"] == true) then
