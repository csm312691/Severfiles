...
RegisterCommand("corner", function()
    ...
    local src = source
    TriggerEvent('cr-drugs:client:cornerselling', src)
end)

RegisterNetEvent('cr-drugs:client:cornerselling', function(data)
    QBCore.Functions.TriggerCallback('cr-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    QBCore.Functions.Notify('You are corner selling drugs.')
                    startLocation = GetEntityCoords(PlayerPedId())
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    QBCore.Functions.Notify('Sales Ended.', "error")
                end
            else
                QBCore.Functions.Notify('Missing Drugs.', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
...
RegisterNetEvent('cr-drugs:client:refreshAvailableDrugs', function(items)
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            pedDist = #(coords - pedCoords)
            if getRobbed == 18 or getRobbed == 9 then
                -- TriggerServerEvent('cr-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                ...
                if pedDist < 1.5 and cornerselling then
                    DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[E] to sell drugs '..bagAmount..'x '..currentOfferDrug.label..' for '..randomPrice..' roll(s)? / [G] Decline offer')
                    if IsControlJustPressed(0, 38) then
                        TriggerServerEvent('cr-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, 47) then
                        QBCore.Functions.Notify('Offer refused.', 'error')
...
--                     DrawText3D(pedpos.x, pedpos.y, pedpos.z, "[E] Pick up your stuff")
