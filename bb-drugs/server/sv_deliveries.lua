...
QBCore.Commands.Add("dealergoto", "Teleport To A Dealer (Admin Only)", {{
    ...
    if Config.Dealers[DealerName] ~= nil then
        TriggerClientEvent('cr-drugs:client:GotoDealer', source, Config.Dealers[DealerName])
    else
...
RegisterServerEvent('cr-drugs:server:CreateDealer')
AddEventHandler('cr-drugs:server:CreateDealer', function(DealerData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
