...
const rangeNumbers = (length = 1) => {
    return Array.from({length}, _ => Math.floor(Math.random() * 4))
}
...
function check(){
    ...
    if(game_started && response === answer.join('')){
        ...
        $.post('https://cr-passwordminigame/callback', JSON.stringify({'success': true}));
        setTimeout( function(){
            $(".minigame").fadeOut()
        }, 3000)
    }else{
        ...
        $.post('https://cr-passwordminigame/callback', JSON.stringify({'success': false}));
        setTimeout( function(){
            $(".minigame").fadeOut()
        }, 3000)
    }
